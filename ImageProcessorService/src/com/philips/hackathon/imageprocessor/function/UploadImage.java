 ...
 * image and stores them to a DynamoDB table. It reads the payload as the content of the image and stores it to a S3 bucket.
 */
public class UploadImage implements RequestHandler<ServerlessInput, ServerlessOutput> {
 ...
 // DynamoDB table attribute name for storing the analytics from rekognition.
    ...
    // S3 bucket name for storing image content.
    private static final String IMAGE_BUCKET_NAME = System.getenv("IMAGE_BUCKET_NAME");
    @Override
    public ServerlessOutput handleRequest(ServerlessInput serverlessInput, Context context) {
            ...
            try {
                ...
                attributes.put(IMAGE_TABLE_BUCKET_NAME, new AttributeValue().withS(IMAGE_BUCKET_NAME));
                attributes.put(IMAGE_TABLE_KEY_NAME, new AttributeValue().withS(keyName));
                /**
                 * Add rekognition call and add results to attribute here
                 */
                String rekognitionJson ="";
                attributes.put(IMAGE_TABLE_REKOGNITION, new AttributeValue().withS(rekognitionJson));
                ...
                output.setBody("Successfully inserted image " + attributes.get(IMAGE_TABLE_ID_NAME).getS());
            } catch (Exception e) {
